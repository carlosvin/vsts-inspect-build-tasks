...
class Todo {
    ...
    constructor(todoText: string, file: string) {
    ...
    addTestCase (testSuite) {
        return this.addTestCaseResult(
            testSuite.ele('testcase', {name: `Found TODO at ${this.file}: ${this.todoText}`, id: this.todoText}));
    }

    private addTestCaseResult (testCase) {
...
const getTodos = (str: string, file: string) => {  
    let m;
    const todos: Todo [] = [];
